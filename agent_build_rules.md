...
*Save in repo root. Agent reads Sections B–D. Human acts on Section A first.*

Critical path summary before you fire the CLI:
The only true hard blockers the agent cannot self-resolve are the texture crops and normal maps from the book photo — everything else (scaffold, install, animate, deploy config) it handles autonomously. Get the assets folder populated from Section A1, run the skills pre-install line, then fire the interactive claude --dangerously-skip-permissions session and paste the Section B prompt. The agent takes it from there through all 7 phases, logging progress to BUILD_LOG.md as it goes.wait.. should that replace my current FULL_bried.md file or be a new file1 step1 stepIt should be a new file. Do not replace anything.
Here's your current root directory state and what each file is:
FileWhat it isKeep?initial_website_plan.mdThe original scrollytelling design concept — scroll sequence, colour palette, chapter structure, animation beats✅ Keep as-isagent_build_rules.mdThe technology rulebook from the Instagram research sweep — stack decisions, phase order, performance rules, excluded tools✅ Keep as-isFULL_brief.mdNew file — the agent launch document. Wraps the other two into an executable brief: prerequisites, the agent prompt, CLAUDE.md content, and the CLI command✅ New, don't replace anything
The reason to keep all three separate is intentional. The agent's prompt in FULL_brief.md literally tells it to read initial_website_plan.md and read agent_build_rules.md as its first action — so those two files need to exist independently in the root for the agent to consume them as context. FULL_brief.md is the launcher that points at the other two, not a replacement for them.
So your root should have all three sitting alongside each other:
/
  initial_website_plan.md   ← design bible
  agent_build_rules.md      ← tech rules
  FULL_brief.md             ← agent launcher (new)
  /assets/
    /textures/
    book-reference-4x.jpgDirectory of C:\Users\marku\OneDrive\Documents\10. AI Projects\Ethan\Prep23/05/2026  08:57 PM    <DIR>          .23/05/2026  07:55 PM    <DIR>          ..23/05/2026  07:56 PM           468,702 custom_binding-1.jpeg23/05/2026  08:57 PM            43,034 FULL_Brief.md               2 File(s)        511,736 bytes               2 Dir(s)  79,944,257,536 bytes free---This is all I have currently, so, tell me what I should save the above as and if any other files need to be save for Agent use to achieve our goals here1 step1 stepGood — your directory is clean. Here's exactly what you have, what's missing, and what to save where.
Current State
C:\Users\marku\OneDrive\Documents\10. AI Projects\Ethan\Prep\
  custom_binding-1.jpeg     ← the book photo ✅
  FULL_Brief.md             ← the agent launcher ✅
What's Missing — Save These Now
You need 3 more .md files saved in this same folder. All three exist already — they were written in this session. Here's what to name each and where the content comes from:

File 1 — Save as: initial_website_plan.md
This is the very first long response I gave you — the one that covered:

The book visual design system (colour tokens, construction details)
The full scroll sequence beat-by-beat (Scene 0 through Scene 6)
The tassel animation detail
The Squarespace integration strategy
The full tech stack and file architecture

Scroll back in this chat to that response and copy the entire thing into a .md file.

File 2 — Save as: agent_build_rules.md
This is the Instagram research output — the long structured document with:

The decision matrix table
The 4-layer Claude Code harness instructions
The phase-by-phase build sequence
Performance rules (P1–P10)
The exclusion table
The creator intelligence sources

Scroll back to that response and copy it.

File 3 — FULL_Brief.md — you already have this ✅

Also Rename Your Image
Your book photo is currently custom_binding-1.jpeg. Rename it to match what the agent expects:
custom_binding-1.jpeg  →  book-reference.jpg
(or keep the name but update the reference in FULL_Brief.md — the agent_build_rules.md refers to it as book-reference-4x.jpg for the upscaled version, but the original can stay as-is until you upscale it in the Section A1 prerequisite step)

Final Target State
C:\Users\marku\OneDrive\Documents\10. AI Projects\Ethan\Prep\
  custom_binding-1.jpeg         ← book photo (original, keep)
  initial_website_plan.md       ← save from this chat
  agent_build_rules.md          ← save from this chat
  FULL_Brief.md                 ← already exists ✅
That's everything. Once those three .md files are in the same folder alongside the book photo, the directory is complete and you're ready for the Section A prerequisites (texture generation, upscaling, repo setup) before firing the CLI command.I just dont have this so give it again to download as an md file here: agent_build_rules.md ← save from this chatHere it is — copy everything between the triple backticks into a file named agent_build_rules.md:
...
- Colour palette is extracted ONLY from the provided book photograph
- Do not use generic "leather brown" or "forest green" approximations
- The exact hex values defined in this document are the law
- All 3D model proportions must match the book's actual aspect ratio
  (approx. 1:1 square-ish, thick spine, bevelled corners)
- Textures must be derived from or faithful to the actual photo:
  cognac pebble-grain leather, fine-weave linen cloth, 5 raised bands
...
nextlevelbuilder/ui-ux-pro-max-skill
Asset generation: Higgsfield MCP (scroll animation frames only)
Design ref tool:  refero.design (before every new page/section)
DEVELOPMENT TOOLING
Version control:  Git + GitHub
Package manager:  pnpm (faster than npm, better for monorepos)
Linting:          ESLint + Prettier
...
# Smooth scroll (mobile-safe, replaces locomotive)
pnpm add lenis
...
**GitHub repos to reference (do not npm install — use as texture/asset
sources and code reference only):**
github.com/ruucm/shadergradient          → Cinematic bg gradients
github.com/paper-design/liquid-logo      → Liquid logo reveal
github.com/dashersw/liquid-glass-js      → Glass morphing (desktop)
github.com/pmndrs/react-three-fiber      → 3D canvas in React
...
2. Crop these exact regions as separate files into `/assets/textures/`:
   - `leather-grain.jpg`     → spine + corner close-up crop
   - `leather-corner.jpg`    → one corner showing chamfer angle
   - `cloth-weave.jpg`       → front board centre crop
   - `spine-bands.jpg`       → spine profile showing the 5 raised ribs
   - `endpaper-marble.jpg`   → generate via Midjourney (see FULL_Brief.md)
3. Run each crop through NormalMap Online (cpetry.github.io/NormalMap-Online/)
   → save as `leather-grain-normal.jpg`, `cloth-weave-normal.jpg`
4. Extract hex values from photo using eyedropper on the upscaled image
   and confirm they match the colour tokens below within ±5 lightness
5. Write `/src/styles/tokens.css` with ALL colour + spacing + font tokens
   BEFORE any component is written

**Colour tokens (validate all against reference photo):**
```css
:root {
  --leather:        #C2813A;
  --leather-dark:   #8B5A24;
...
**Goal:** GSAP ScrollTrigger wired to the scene, Lenis smooth scroll
installed, mobile/desktop modes confirmed.

Steps:
1. Install and configure Lenis:
```typescript
   const lenis = new Lenis({ lerp: 0.08, smoothWheel: true })
   gsap.ticker.add((time) => lenis.raf(time * 1000))
   gsap.ticker.lagSmoothing(0)
...
```typescript
   ScrollTrigger.matchMedia({
     "(min-width: 768px)": function() {
       createPinnedSections()      // desktop: pin-based
     },
     "(max-width: 767px)": function() {
       createMobileScrollScene()   // mobile: opacity/transform only
     }
   })
```
...
**Goal:** The book cover swings open on scroll. Most important animation.
...
```
2. GSAP ScrollTrigger drives rotateY from 0° to -160°:
```typescript
   gsap.to(".book-cover", {
     rotateY: -160,
     ease: "none",
     scrollTrigger: {
       ...
       scrub: 1.5,
       pin: true  // desktop only — wrapped in matchMedia
     }
   })
```
3. Specular highlight sweeps across hinge as it opens
4. Endpaper reveal: marbled parchment + handwritten quote

**DONE CONDITION:** Cover opens smoothly on desktop with pin. On mobile,
cover fades with 30° tilt (no full pin). Hinge highlight fires correctly.
...
2. `TasselBookmark.tsx` SVG structure:
   ...
   - Fringe ends (tiny SVG teardrops)
3. Idle micro-drift: Perlin noise applied to angle when scrollDelta ≈ 0
4. Tassel repositions between chapters as scroll bookmark

**DONE CONDITION:** Tassel swings naturally on fast scroll, sways on
slow scroll, hangs still with micro-drift at rest. No library needed.
Renders correctly at 390px mobile width.
...
Assemble chapters IN ORDER per `initial_website_plan.md`.
For each chapter:
1. Pull component starting points from 21st.dev
2. Run /polish skill after building each chapter
3. Cross-reference Refero.design for any uncertain UI pattern
4. Validate all colours against tokens.css
5. Test on mobile after EACH chapter before proceeding

Chapter order:
  Chapter 0: Hero (book idle + title + wordmark)
  Chapter 1: Cover Open (endpaper reveal + handwritten quote)
  Chapter 2: About (first page turn + Ethan's story)
  Chapter 3: Services (pinned book + stacking service cards)
  Chapter 4: Process (horizontal scroll — 4 illustrated steps)
  Chapter 5: Workshops (video panel + wax seal CTA button)
  Chapter 6: Contact (book closes + back cover contact details)
...
For each chapter:
1. Test on 390px viewport in Chrome DevTools mobile emulation
2. Test on Safari iOS (BrowserStack if no physical device available)
3. Confirm Three.js fallback renders correctly on mobile
4. Check: no horizontal scroll overflow
5. Check: tap targets ≥ 44×44px (WCAG 2.5.5)
6. Check: text contrast ratio ≥ 4.5:1 (WCAG 1.4.3)
7. Run Lighthouse mobile audit — score must be ≥ 85 performance
8. Fix ALL issues before Phase 7

---

### PHASE 7 — Vercel Config + DNS + Handoff

Steps:
1. Configure Vercel project for root domain
2. Generate `DNS_INSTRUCTIONS.md` with exact A/CNAME record values
3. Audit all CTA links point to shop.walkerbookworks.com.au
4. Set up Vercel Analytics (free tier)
5. Write `HANDOFF.md` for Ethan:
   - How to update Squarespace (normal, unchanged workflow)
   - Which files to edit for copy changes
   - Contact for frontend changes

**Note:** Agent generates DNS instructions only. Human adds DNS records
manually. Agent never touches the Squarespace account.
...
2. Search for the most similar existing high-end site pattern
3. Screenshot 2–3 references
4. Include them in Claude Code context with the prompt:
   "Build [component] with this visual quality and interaction density.
   Apply Walker Book Works colour tokens and typography. Book-craft
   aesthetic, not tech startup aesthetic."
...
Enforced behaviours, not suggestions:
RULE P1:  No Three.js on mobile. CSS/SVG fallback always.
RULE P2:  No autoplay video above the fold on mobile.
RULE P3:  All images: next/image with WebP, lazy below fold.
RULE P4:  No GSAP pin on mobile. Scroll-driven opacity/transform only.
RULE P5:  Lenis lerp: 0.08 desktop / native scroll on mobile.
RULE P6:  Font loading: font-display: swap on all Google Fonts.
RULE P7:  Texture files: compress to < 200kb each (WebP format).
RULE P8:  Code-split Three.js: dynamic import with Suspense boundary.
RULE P9:  Total page weight: < 1MB on initial load.
RULE P10: Scroll listener debounce: 16ms (one frame) minimum.
...
| Framer (no-code) | Insufficient control for custom book physics |
| Runable | Good for fast prototypes, not production custom builds |
| Durable | Fully automated, no custom code — wrong tool |
| html-in-canvas | Experimental Chrome API, breaks Firefox/Safari/mobile |
| MCP servers (general) | Consume context window; CLI tools preferred |
| Tailwind CSS | Conflicts with fine-grained animation token system |
| Bootstrap / MUI | Wrong aesthetic for handcraft bookbinding site |
| Any parallel SaaS builder | Client paid for Squarespace — no parallel tools |
...
BookPage.tsx            ← Individual page with curl
TasselBookmark.tsx      ← Physics tassel (ribbon, knot, threads, fringe)
BookIllustration.tsx    ← CSS/SVG fallback for mobile
DustParticles.tsx       ← Ambient gold dust motes (desktop only)
/Chapters
...
useDeviceCapability.ts    ← Routes 3D vs CSS/SVG based on device
/styles
tokens.css                ← All colour/font/spacing tokens
book.css                  ← Book-specific 3D CSS rules
/assets
book-reference-4x.jpg       ← Upscaled source photo (design bible)
/textures
leather-grain.jpg
leather-grain-normal.jpg
leather-corner.jpg
...
endpaper-marble.jpg
/video
book-pages-cinematic.mp4  ← (optional) Higgsfield generated frames

---

## SECTION 9 — CREATOR INTELLIGENCE SOURCES

Instagram accounts whose content informed this document:
...
- [ ] HANDOFF.md written and reviewed by Ethan
- [ ] DNS configured: root → Vercel, shop → Squarespace
- [ ] Squarespace password page removed (Ethan's action at launch)

---

*End of agent_build_rules.md*
*Cross-reference: initial_website_plan.md · FULL_Brief.md · /assets/book-reference-4x.jpg*
